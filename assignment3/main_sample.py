...
def data_split(data_name, sparse_bool):
    if data_name == 'heart':
        ...
        data_size = X.shape[0]
        train_size = np.int(np.floor(data_size / 10) * 7)
        val_size = np.int(np.floor(data_size / 10))
        test_size = np.int(data_size - train_size - val_size)
        ...
        val_num = np.int(np.floor(X.shape[0] / 2))
        X_val = X[0:val_num]
        Y_val = Y[0:val_num]
        X_test = X[val_num:]
        Y_test = Y[val_num:]
